...
%Solve the LP problem in Cplex
        ...
        %% With the superedges this should not be necessary.
        if strcmp(sol.Solution,'infeasible')
            ...
            while inf_counter < 11
                ...
                [nsp,csp] = getsp_s(fd,td,dd,s,t); %% This should be performed in modified cost space, first trial there are none calulated
                ...
                if strcomp(sol.Solution,'infeasible')
        ...
        else % If solution was found, move on
            
            [link_flows, link_duals, comm_duals,se_flows, D, F] = sol_handle_s(sol,dij,linklist,nl,nn,pathcosts,kpath);%Handles the solution vector, considering feasibility 
...
disp('Number of Infeasible problems')
sum(Fvec)
