...
GRC = digraph(link_res(:,1),link_res(:,2),link_res(:,3)-link_res(:,5));

%Define characteristics
LWidths = 5*GR.Edges.Weight/max(GR.Edges.Weight) + 1;
...
p1 = plot(GR,'XData',xlocation,'YData',ylocation,...
    ...
    p1.MarkerSize = 10*(outgoing + incoming)./max(abs(outgoing + incoming)) + 1;
